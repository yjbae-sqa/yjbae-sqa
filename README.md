# Quality Engineer | Test Automation | API Testing | CI/CD

## 👋 안녕하세요!
Web / Mobile / API 환경에서 테스트 자동화와 품질 프로세스 개선을 중심으로 서비스 안정성을 구축해온 Quality Engineer입니다.  
개발 경험을 바탕으로 시스템 구조를 이해하고 비즈니스 리스크를 기준으로 검증 범위와 우선순위를 정하여 API 검증, 서비스 흐름 테스트, 자동화 전략 및 검증 구조를 설계하는 Technical QA 역할에 강점을 가지고 있습니다.  
특히, 복잡한 QA 상황을 정리하고 테스트 자동화, CI/CD 기반 품질 검증 체계를 구조적으로 구축해왔습니다.  
조직에서 QA 프로세스 정리, 테스트 자동화 체계 구축, CI/CD 기반 품질 검증이 필요하시다면 [LinkedIn](https://www.linkedin.com/in/yjbae/)으로 연락 부탁드립니다. ☕

## 🧩 Quality Engineering

### CI/CD & Automation
- Docker 기반 테스트 환경에서 자동화 테스트 실행 구조 및 검증 프로세스 설계/운영
- CI/CD 파이프라인과 자동화 테스트를 연동하여 배포 전 검증 프로세스 구축

### Release Quality
- 배포 리스크를 줄이기 위해 품질 지표 기반 Quality Gate 기준 정의 및 검즈 프로세스 정립
- Release 전 핵심 기능 안정성을 확보하기 위해 Smoke 테스트 전략 적용

### QA Process & Technical Testing
- 개발 조직 협업 기반으로 테스트 프로세스 및 배포 검증 흐름 설계/개선
- 서비스 구조와 API 호출 흐름을 기반으로 End-to-End 검증 구조 설계
- 초기 조직에서 QA 프로세스 및 테스트 운영 체계 정의 및 구축
...
- 2026년도 상반기 (In Progress) : CI/CD Pipeline과 Test Automation 중심의 QA 사이드 프로젝트 진행 중

## ✅ Testing
### Service Flow Testing
- 사용자 영향도와 비즈니스 리스크 기준으로 End-to-End 검증 범위 및 시나리오 정의
- API 호출 흐름과 서비스 동작을 기반으로 통합 기능 검증 구조 설계

### API Testing
- 서비스 데이터 흐름과 데이터 정합성을 기준으로 API 검증 포인트 및 응답 검증 기준 정의  
- API Spec 기반 기능 및 통합 검증 구조 설계

### Light Performance Testing
- API 응답 시간 및 처리 한계를 기준으로 성능 검증 범위 정의 및 간단한 부하 테스트 설계
- Newman 기반 API 테스트 실행을 활용한 성능 및 스트레스 테스트 검증 수행

### Test Automation
- 반복 검증 효율성과 유지보수성을 기준으로 UI 테스트 자동화 구조 설계 (Selenium / Appium) 
- TestNG 기반 테스트 실행 구조 및 리포트
...
[![Gemini](https://skills.syvixor.com/api/icons?perline=15&i=geminicli)](https://geminicli.com/)
...
## 🌐 Domains
[![AI](https://img.shields.io/badge/AI-8A2BE2?style=for-the-badge&logoColor=white)](https://namu.wiki/w/%EC%9D%B8%EA%B3%B5%EC%A7%80%EB%8A%A5)
[![MLOps](https://img.shields.io/badge/MLOps-0FA3B1?style=for-the-badge&logoColor=white)](https://www.databricks.com/kr/glossary/mlops)
[![E-Commerce](https://img.shields.io/badge/E_Commerce-F57C00?style=for-the-badge&logoColor=white)](https://en.wikipedia.org/wiki/E-commerce)
[![Computer Vision](https://img.shields.io/badge/ComputerVision-6C63FF?style=for-the-badge&logoColor=white)](https://namu.wiki/w/B2B)
[![Visualization Systems](https://img.shields.io/badge/VisualizationSystems-1E90FF?style=for-the-badge&logoColor=white)](https://namu.wiki/w/B2B)
